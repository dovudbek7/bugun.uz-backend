"""Auto-translate category titles on save (admin, API, shell — all paths)."""
...
def _cache_old_title(sender, instance, **kwargs):
    if instance.pk:
        old = Category.objects.filter(pk=instance.pk).only("title").first()
...
def _translate_on_save(sender, instance, created, update_fields=None, **kwargs):
    ...
    if created or old != instance.title:
        translate_taxonomy_obj.delay("categories", "Category", instance.pk)
